...
test.describe('NeonHub Complete Workflow E2E Tests', () => {
  ...
  test('should complete agent creation and execution workflow', async ({ page }) => {
    // Login first
    ...
    await page.fill('[data-testid="agent-description-input"]', 'A test agent for content generation');
    await page.selectOption('[data-testid="agent-type-select"]', 'CONTENT_CREATOR');
    
    // Configure agent settings
    await page.fill('[data-testid="agent-config-topic"]', 'AI Marketing Trends');
    await page.selectOption('[data-testid="agent-config-tone"]', 'professional');
    ...
    await expect(page.locator('[data-testid="agent-list"]')).toContainText('Test Content Agent');
    
    // Run the agent
    await page.click('[data-testid="run-agent-button"]');
    
    // Verify real-time status updates
    await expect(page.locator('[data-testid="agent-status"]')).toContainText('RUNNING', { timeout: 5000 });
    ...
    await expect(page.locator('[data-testid="execution-logs"]')).toContainText('Content generation completed');
  });

  test('should complete campaign creation and management workflow', async ({ page }) => {
    // Login
    await page.goto('/auth/login');
    await page.fill('[data-testid="email-input"]', 'test@neonhub.ai');
    await page.fill('[data-testid="password-input"]', 'SecurePassword123!');
    ...
    // Fill campaign form
    await page.fill('[data-testid="campaign-name"]', 'AI Product Launch Campaign');
    await page.fill('[data-testid="campaign-description"]', 'Comprehensive campaign for AI product launch');
    await page.selectOption('[data-testid="campaign-type"]', 'INTEGRATED');
    await page.fill('[data-testid="campaign-budget"]', '50000');
    await page.fill('[data-testid="target-audience"]', 'Tech professionals and early adopters');
    await page.fill('[data-testid="campaign-goals"]', 'Generate 1000 leads and increase brand awareness by 30%');
    ...
    await expect(page).toHaveURL('/marketing/campaigns');
    await expect(page.locator('[data-testid="campaign-list"]')).toContainText('AI Product Launch Campaign');
    
    // Open campaign details
    await page.click('[data-testid="campaign-item"]:has-text("AI Product Launch Campaign")');
    
    // Verify campaign details are displayed
    await expect(page.locator('[data-testid="campaign-status"]')).toContainText('DRAFT');
    await expect(page.locator('[data-testid="campaign-budget"]')).toContainText('$50,000');
  });
  ...
  test('should display real-time analytics and metrics', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="agent-metrics-chart"]')).toBeVisible();
  ...
  test('should complete content generation workflow', async ({ page }) => {
    ...
    await page.fill('[data-testid="content-title"]', 'The Future of AI in Marketing');
    ...
    await page.fill('[data-testid="target-audience"]', 'Marketing professionals and business leaders');
    await page.fill('[data-testid="key-points"]', 'AI automation, personalization, data-driven insights');
    await page.fill('[data-testid="keywords"]', 'AI marketing, automation, personalization');
    ...
    await expect(page.locator('[data-testid="content-metadata"]')).toContainText('words');
  ...
  test('should handle error scenarios gracefully', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="error-state"]')).toContainText('Failed to load');
    
    // Should have retry button
    await expect(page.locator('[data-testid="retry-button"]')).toBeVisible();
  });

  test('should support responsive design on mobile', async ({ page, browserName }) => {
    // Set mobile viewport
    ...
    await expect(page.locator('[data-testid="agent-grid"]')).toHaveCSS('grid-template-columns', /1fr/);
  });
}); 