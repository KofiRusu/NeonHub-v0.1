...
const LOG_DIR = path.resolve(__dirname, config.LOG_DIR);
const LOG_FILE = path.join(LOG_DIR, config.LOG_FILE);
...
function logEvent(source: string, type: string, message: string) {
...
// Read recent lines
function getRecentEvents(limit = 20): string[] {
...
app.post('/events', (req, res) => {
  const { source, type, message } = req.body as { source: string; type: string; message: string };
...
setInterval(() => {
  ...
  if (!projDone || !ciDone) {
    // Log any blockers
    const blockers = [];
    if (!projDone) blockers.push('Project not complete');
    if (!ciDone) blockers.push('CI checks not passed');
  ...
  if (projDone && ciDone && !hasDeployed) {
    ...
    try {
      ...
      fs.copyFileSync(LOG_FILE, `${LOG_FILE}.${timestamp}.backup`);
    } catch (err) {
      // Log deployment failure
      logEvent('ORCHESTRATOR', 'DEPLOY_ERROR', (err as Error).message);
