...
describe('AgentScheduler', () => {
  ...
  describe('constructor', () => {
    ...
    it('should auto-start when autoStart is true', () => {
      ...
      const autoStartScheduler = new AgentScheduler(mockPrisma, mockAgentManager, {
        checkInterval: 1000,
        autoStart: true
      });
  ...
  describe('scheduleAgent', () => {
    it('should schedule an agent successfully', async () => {
      ...
      await scheduler.scheduleAgent('test-agent-id', cronExpression, AgentPriority.HIGH);
    ...
    it('should unschedule agent when enabled is false', async () => {
      ...
      await scheduler.scheduleAgent('test-agent-id', cronExpression, AgentPriority.NORMAL, false);
    ...
    it('should throw error for invalid cron expression', async () => {
      ...
      await expect(scheduler.scheduleAgent('test-agent-id', invalidCron)).rejects.toThrow(
  ...
  describe('priority handling', () => {
    it('should assign correct priority based on agent type', async () => {
      ...
      mockPrisma.aIAgent.findMany.mockResolvedValue([customerSupportAgent, contentCreatorAgent]);
  ...
  describe('error handling and retries', () => {
    ...
    it('should remove agent from schedule after max retries', async () => {
      ...
      const errorStatusUpdate = updateCalls.find(call => 
        call[0].data && call[0].data.status === 'ERROR'
      );
  ...
  describe('missed jobs handling', () => {
    it('should run missed jobs on startup when enabled', async () => {
      ...
      const schedulerWithMissedJobs = new AgentScheduler(mockPrisma, mockAgentManager, {
        checkInterval: 1000,
