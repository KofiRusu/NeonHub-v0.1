...
describe('AgentManager', () => {
  ...
  describe('startAgent', () => {
    it('should check if agent is already running and return early if it is', async () => {
      ...
      expect(result).toEqual({ status: 'already_running', agentId: 'agent-id' });
    ...
    it('should throw error if agent not found', async () => {
      ...
      await expect(agentManager.startAgent('non-existent-id'))
  ...
  describe('stopAgent', () => {
    ...
    it('should stop a running agent and update its status', async () => {
      ...
      jest.spyOn(agentManager, 'isAgentRunning')
      ...
      (agentManager as any).runningAgents = new Map([
        [agentId, mockBaseAgent]
      ]);
