...
import { FeedbackChannel, FeedbackSource, Sentiment } from '@prisma/client';
...
export const getAllFeedback = async (req: Request, res: Response) => {
  try {
    ...
    if (sourceType) {
      where.sourceType = sourceType as FeedbackSource;
    }

    // Filter by source ID
    if (sourceId) {
      where.sourceId = sourceId as string;
    }
    ...
    // Filter by channel
    if (channel) {
      where.channel = channel as FeedbackChannel;
    }
