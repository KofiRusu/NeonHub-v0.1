...
export class PerformanceOptimizerAgent extends BaseAgent {
   ...
   */
  private async fetchCampaignMetrics(campaignId: string, platforms: Platform[]): Promise<any[]> {
  ...
  private generateSimulatedMetrics(platform: Platform): any {
    ...
    switch(platform) {
      case 'FACEBOOK':
        ...
        baseMetrics.conversions = Math.floor(baseMetrics.clicks * (0.05 + Math.random() * 0.10));
        return {
          ...baseMetrics,
   ...
   */
  private async generateOptimizations(currentMetrics: any[], config: PerformanceConfig): Promise<any[]> {
    // Check if agent should stop
    if (this.checkShouldStop()) {
   ...
   */
  private async storeOptimizationResults(campaignId: string, optimizations: any[]): Promise<any[]> {
    const storedMetrics = [];
   ...
   */
  private generateOptimizationSummary(currentMetrics: any[], optimizations: any[], config: PerformanceConfig): string {
    // Calculate totals
    ...
    const highPriority = optimizations.reduce((count, platformOpt) => {
      return count + platformOpt.recommendations.filter((r: any) => r.importance === 'HIGH').length;
    }, 0);
    
    const mediumPriority = optimizations.reduce((count, platformOpt) => {
      return count + platformOpt.recommendations.filter((r: any) => r.importance === 'MEDIUM').length;
    }, 0);
...
- CTR: +${((optimizations.reduce((sum, platformOpt) => {
    return sum + platformOpt.recommendations
      .filter((r: any) => r.estimatedImprovement?.metric === 'CTR')
      .reduce((sum: number, r: any) => sum + (r.estimatedImprovement?.value || 0), 0);
  }, 0) * 100).toFixed(2))}%
- CPC: -$${(optimizations.reduce((sum, platformOpt) => {
    return sum + platformOpt.recommendations
...
- Conversion Rate: +${((optimizations.reduce((sum, platformOpt) => {
    return sum + platformOpt.recommendations
      .filter((r: any) => r.estimatedImprovement?.metric === 'ConversionRate')
      .reduce((sum: number, r: any) => sum + (r.estimatedImprovement?.value || 0), 0);
  }, 0) * 100).toFixed(2))}%`;
  }
} 