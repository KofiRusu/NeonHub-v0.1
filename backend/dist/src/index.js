...
exports.getAgentScheduler = exports.prisma = void 0;
...
const agentEvents_1 = require('./socket/agentEvents');
...
const agentIo = (0, agentOutput_1.configureAgentSocket)(
  httpServer,
  exports.prisma,
);
...
async function initializeScheduler() {
  try {
    await schedulerSingleton_1.schedulerSingleton.initialize();
...
(0, agentEvents_1.initializeAgentEvents)(io);
// Export scheduler helper for use in routes
const getAgentScheduler = () =>
  schedulerSingleton_1.schedulerSingleton.getScheduler();
  ...
  exports.prisma.$queryRaw`SELECT 1`
    .then(() => {
      res.status(200).json({
        ...
        schedulerHealthy: schedulerSingleton_1.schedulerSingleton.isHealthy(),
      });
    })
    .catch((err) => {
...
process.on('SIGINT', async () => {
  await schedulerSingleton_1.schedulerSingleton.stop();
