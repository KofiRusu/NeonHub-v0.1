...
class BaseAgent {
    ...
    async execute(config, options = {}) {
        ...
        try {
            ...
            const campaign = await campaignService.getOrCreateCampaignForAgent(this.agentData, linkedCampaignId);
            ...
            if (trackMetrics) {
                ...
                await metricService.logAgentExecutionMetrics(executionTime, this.agentData.id, this.agentData.agentType, this.agentData.projectId, session.id, campaign.id, true, options.tokenUsage);
            }
            // Log execution completion event
            this.logEvent(AgentEventType.EXECUTION_COMPLETED, `Agent execution completed successfully in ${executionTime}ms`, { executionTime, sessionId: session.id });
            ...
            this.logEvent(AgentEventType.EXECUTION_FAILED, `Agent execution failed: ${error instanceof Error ? error.message : String(error)}`, { error: error instanceof Error ? error.stack : error });
            // Calculate execution duration
            const executionTime = Date.now() - this.executionStartTime;
            ...
            if (this.currentSessionId) {
                ...
                if (trackMetrics) {
                    try {
                        const metricService = (0, services_1.getMetricService)(this.prisma);
                        await metricService.logAgentExecutionMetrics(executionTime, this.agentData.id, this.agentData.agentType, this.agentData.projectId, this.currentSessionId, campaignId, false, options.tokenUsage);
                    }
                    catch (metricError) {
    ...
    async executeWithRetry(config) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            ...
            try {
                if (attempt > 0) {
                    this.logEvent(AgentEventType.RETRY_ATTEMPT, `Retry attempt ${attempt}/${this.maxRetries}`, { attempt, maxRetries: this.maxRetries });
                ...
                // Log retry event
                this.logEvent(AgentEventType.RETRY_ATTEMPT, `Attempt ${attempt + 1} failed, will retry: ${lastError.message}`, { attempt: attempt + 1, error: lastError.message }, 'warning');
            }
        }
