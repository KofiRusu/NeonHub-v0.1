...
class AudienceResearchAgent extends BaseAgent_1.BaseAgent {
    ...
    async executeImpl(config) {
        ...
        try {
            ...
            const audienceSegments = await this.generateAudienceSegments(demographicData, behavioralData, competitorData);
            // Store the research results as trend signals
            const signals = await Promise.all(audienceSegments.map((segment) => this.prisma.trendSignal.create({
                data: {
     ...
     */
    async generateAudienceSegments(demographicData, behavioralData, competitorData) {
        ...
        const selectedArchetypes = this.selectRelevantArchetypes(archetypes, demographicData, behavioralData);
...
4. Consider deeper research on the ${topSegments[0].name} segment`;
    ...
    generateChannelPreferences(industry) {
        return [
            ...
            {
                channel: 'LinkedIn',
                percentage: industry.toLowerCase() === 'technology' ||
    ...
    generateMessagingRecommendations(archetype) {
        ...
        return {
            ...
            tone: archetype.name === 'Early Adopters'
...
//# sourceMappingURL=AudienceResearchAgent.js.map