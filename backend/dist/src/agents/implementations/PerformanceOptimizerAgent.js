...
class PerformanceOptimizerAgent extends BaseAgent_1.BaseAgent {
  ...
  async fetchCampaignMetrics(campaignId, platforms) {
    ...
    await this.logMessage(
      'info',
      `Fetching metrics for campaign ${campaignId}...`,
    );
  ...
  async generateOptimizations(currentMetrics, config) {
    ...
    await this.logMessage(
      'info',
      `Analyzing metrics and generating optimizations...`,
    );
    ...
    for (const platformOptimization of optimizations) {
      ...
      const optimizationRecord = await this.prisma.metric.create({
        data: {
          ...
          campaignId,
          metadata: platformOptimization.recommendations,
          projectId:
            (
              await this.prisma.campaign.findUnique({
                where: { id: campaignId },
  ...
  generateOptimizationSummary(currentMetrics, optimizations, config) {
    ...
    const mediumPriority = optimizations.reduce((count, platformOpt) => {
      return (
        count +
        platformOpt.recommendations.filter((r) => r.importance === 'MEDIUM')
          .length
      );
    }, 0);
...
- Conversion Rate: +${(
  ...
  async stopImpl() {
    await this.logMessage(
      'info',
      'Stopping performance optimizer agent execution',
    );
  }
}
