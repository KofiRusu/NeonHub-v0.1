...
exports.PerformanceOptimizerAgent = void 0;
const BaseAgent_1 = require("../base/BaseAgent");
/**
 * Agent for analyzing marketing metrics and optimizing campaigns
 */
class PerformanceOptimizerAgent extends BaseAgent_1.BaseAgent {
    ...
    async executeImpl(config) {
        ...
        await this.logMessage(`Starting performance optimization for campaign ${config.campaignId}`);
        ...
        try {
            ...
            const currentMetrics = await this.fetchCampaignMetrics(config.campaignId, config.platforms);
            // Generate optimization recommendations
            const optimizations = await this.generateOptimizations(currentMetrics, config);
            // Store optimization results as metrics
            const storedMetrics = await this.storeOptimizationResults(config.campaignId, optimizations);
            await this.logMessage(`Created ${storedMetrics.length} optimization recommendations`);
            return {
                status: 'success',
                currentMetrics,
                optimizations,
                summary: this.generateOptimizationSummary(currentMetrics, optimizations, config),
            };
        }
        catch (error) {
            await this.logMessage(`Error in performance optimizer: ${error instanceof Error ? error.message : String(error)}`, 'error');
            throw error;
        }
    }
    /**
     ...
     */
    async fetchCampaignMetrics(campaignId, platforms) {
     ...
     */
    generateSimulatedMetrics(platform) {
        ...
        switch (platform) {
            case 'FACEBOOK':
                baseMetrics.clicks = Math.floor(baseMetrics.impressions * (0.01 + Math.random() * 0.03));
                baseMetrics.conversions = Math.floor(baseMetrics.clicks * (0.05 + Math.random() * 0.1));
                return {
                    ...baseMetrics,
                    ...
                    reach: Math.floor(baseMetrics.impressions / (2.1 + Math.random() * 1.5)),
                };
            case 'GOOGLE':
                baseMetrics.clicks = Math.floor(baseMetrics.impressions * (0.02 + Math.random() * 0.04));
                baseMetrics.conversions = Math.floor(baseMetrics.clicks * (0.08 + Math.random() * 0.12));
                return {
                    ...baseMetrics,
                    ctr: baseMetrics.clicks / baseMetrics.impressions,
                    cpc: baseMetrics.spend / baseMetrics.clicks,
            ...
            case 'LINKEDIN':
                baseMetrics.clicks = Math.floor(baseMetrics.impressions * (0.005 + Math.random() * 0.02));
                baseMetrics.conversions = Math.floor(baseMetrics.clicks * (0.04 + Math.random() * 0.08));
                return {
                    ...baseMetrics,
                    ctr: baseMetrics.clicks / baseMetrics.impressions,
                    cpc: baseMetrics.spend / baseMetrics.clicks,
                    ...
                    companyPageClicks: Math.floor(baseMetrics.clicks * (0.2 + Math.random() * 0.3)),
                };
            case 'TWITTER':
                baseMetrics.clicks = Math.floor(baseMetrics.impressions * (0.008 + Math.random() * 0.025));
                baseMetrics.conversions = Math.floor(baseMetrics.clicks * (0.02 + Math.random() * 0.06));
                return {
                    ...baseMetrics,
                    ctr: baseMetrics.clicks / baseMetrics.impressions,
                    cpc: baseMetrics.spend / baseMetrics.clicks,
                    ...
                    retweets: Math.floor(baseMetrics.clicks * (0.05 + Math.random() * 0.1)),
     ...
     */
    async generateOptimizations(currentMetrics, config) {
        // Check if agent should stop
        if (this.checkShouldStop()) {
            ...
            switch (platform) {
                case 'FACEBOOK':
                    platformOptimizations.push({
                        type: 'AUDIENCE',
                        importance: 'MEDIUM',
                        description: 'Expand lookalike audience to increase reach while maintaining conversion rates',
                        estimatedImprovement: {
                            metric: 'Reach',
                ...
                case 'LINKEDIN':
                    platformOptimizations.push({
                        type: 'CREATIVE',
                        importance: 'MEDIUM',
                        description: 'Add industry-specific case studies to ad content to improve relevance',
                        estimatedImprovement: {
                            metric: 'CTR',
     ...
     */
    async storeOptimizationResults(campaignId, optimizations) {
        const storedMetrics = [];
        for (const platformOptimization of optimizations) {
            ...
            const metricRecord = await this.prisma.metric.create({
                data: {
                    ...
                    campaignId,
                    data: metrics,
                    projectId: (await this.prisma.campaign.findUnique({
                        where: { id: campaignId },
            ...
            const optimizationRecord = await this.prisma.metric.create({
                data: {
                    ...
                    campaignId,
                    data: platformOptimization.recommendations,
                    projectId: (await this.prisma.campaign.findUnique({
                        where: { id: campaignId },
                        select: { projectId: true },
                    }))?.projectId || '',
     ...
     */
    generateOptimizationSummary(currentMetrics, optimizations, config) {
        // Calculate totals
        ...
        const highPriority = optimizations.reduce((count, platformOpt) => {
            return (count +
                platformOpt.recommendations.filter((r) => r.importance === 'HIGH')
                    .length);
        }, 0);
        const mediumPriority = optimizations.reduce((count, platformOpt) => {
            return (count +
                platformOpt.recommendations.filter((r) => r.importance === 'MEDIUM').length);
        }, 0);
...
- CTR: +${(optimizations.reduce((sum, platformOpt) => {
            return (sum +
                platformOpt.recommendations
                    .filter((r) => r.estimatedImprovement?.metric === 'CTR')
                    .reduce((sum, r) => sum + (r.estimatedImprovement?.value || 0), 0));
        }, 0) * 100).toFixed(2)}%
- CPC: -$${optimizations
            .reduce((sum, platformOpt) => {
            return (sum +
                platformOpt.recommendations
                    .filter((r) => r.estimatedImprovement?.metric === 'CPC')
                    .reduce((sum, r) => sum + (r.estimatedImprovement?.value || 0), 0));
        }, 0)
            .toFixed(2)}
- Conversion Rate: +${(optimizations.reduce((sum, platformOpt) => {
            return (sum +
                platformOpt.recommendations
                    .filter((r) => r.estimatedImprovement?.metric === 'ConversionRate')
                    .reduce((sum, r) => sum + (r.estimatedImprovement?.value || 0), 0));
        }, 0) * 100).toFixed(2)}%`;
    }
}
exports.PerformanceOptimizerAgent = PerformanceOptimizerAgent;
//# sourceMappingURL=PerformanceOptimizerAgent.js.map