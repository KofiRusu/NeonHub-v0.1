...
const contentCreatorPlugin = {
    type: client_1.AgentType.CONTENT_CREATOR,
    name: 'Content Creator Agent',
    description: 'Creates engaging content for various platforms and formats',
    version: '1.0.0',
    create: (prisma, agentData) => new ContentCreatorAgent_1.ContentCreatorAgent(prisma, agentData),
...
const trendAnalyzerPlugin = {
    type: client_1.AgentType.TREND_ANALYZER,
    name: 'Trend Analyzer Agent',
    description: 'Analyzes market trends and identifies opportunities',
    version: '1.0.0',
    create: (prisma, agentData) => new TrendAnalyzerAgent_1.TrendAnalyzerAgent(prisma, agentData),
    validateConfig: (config) => {
        return (config &&
            typeof config === 'object' &&
...
const outreachManagerPlugin = {
    type: client_1.AgentType.OUTREACH_MANAGER,
    name: 'Outreach Manager Agent',
    description: 'Manages outreach campaigns and lead generation',
    version: '1.0.0',
    create: (prisma, agentData) => new OutreachManagerAgent_1.OutreachManagerAgent(prisma, agentData),
    validateConfig: (config) => {
        return (config &&
            typeof config === 'object' &&
...
const performanceOptimizerPlugin = {
    type: client_1.AgentType.PERFORMANCE_OPTIMIZER,
    name: 'Performance Optimizer Agent',
    description: 'Optimizes campaign performance and ad spend',
    version: '1.0.0',
    create: (prisma, agentData) => new PerformanceOptimizerAgent_1.PerformanceOptimizerAgent(prisma, agentData),
    validateConfig: (config) => {
        return (config &&
            typeof config === 'object' &&
...
const audienceResearchPlugin = {
    type: client_1.AgentType.AUDIENCE_RESEARCHER,
    name: 'Audience Research Agent',
    description: 'Researches and analyzes target audiences',
    version: '1.0.0',
    create: (prisma, agentData) => new AudienceResearchAgent_1.AudienceResearchAgent(prisma, agentData),
    validateConfig: (config) => {
        return (config &&
            typeof config === 'object' &&
...
const seoPlugin = {
    type: client_1.AgentType.SEO_SPECIALIST,
...
const customerSupportPlugin = {
    type: client_1.AgentType.CUSTOMER_SUPPORT,
    name: 'Customer Support Agent',
    description: 'Provides automated customer support and assistance',
    version: '1.0.0',
    create: (prisma, agentData) => new CustomerSupportAgent_1.CustomerSupportAgent(prisma, agentData),
    validateConfig: (config) => {
        return (config &&
            typeof config === 'object' &&
