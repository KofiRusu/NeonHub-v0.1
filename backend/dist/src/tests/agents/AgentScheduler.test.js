...
describe('AgentScheduler', () => {
    ...
    beforeEach(() => {
        ...
        scheduler = new AgentScheduler_1.AgentScheduler(mockPrisma, mockAgentManager, {
    ...
    describe('constructor', () => {
        it('should create scheduler with default options', () => {
            const defaultScheduler = new AgentScheduler_1.AgentScheduler(mockPrisma, mockAgentManager);
            expect(defaultScheduler).toBeInstanceOf(AgentScheduler_1.AgentScheduler);
        ...
        it('should auto-start when autoStart is true', () => {
            ...
            const autoStartScheduler = new AgentScheduler_1.AgentScheduler(mockPrisma, mockAgentManager, {
                checkInterval: 1000,
                autoStart: true,
            });
    ...
    describe('scheduleAgent', () => {
        it('should schedule an agent successfully', async () => {
            ...
            await scheduler.scheduleAgent('test-agent-id', cronExpression, AgentScheduler_1.AgentPriority.HIGH);
        ...
        it('should unschedule agent when enabled is false', async () => {
            ...
            mockPrisma.aIAgent.update.mockResolvedValue(mockAgent);
            await scheduler.scheduleAgent('test-agent-id', cronExpression, AgentScheduler_1.AgentPriority.NORMAL, false);
            expect(mockPrisma.aIAgent.update).toHaveBeenCalledWith({
                where: { id: 'test-agent-id' },
                data: {
                    scheduleExpression: cronExpression,
        ...
        it('should throw error for invalid cron expression', async () => {
            ...
            await expect(scheduler.scheduleAgent('test-agent-id', invalidCron)).rejects.toThrow('Invalid cron expression: invalid-cron');
        });
    });
    describe('calculateNextRunTime', () => {
        ...
        it('should throw error for invalid cron expression', () => {
            ...
            expect(() => scheduler.calculateNextRunTime(invalidCron)).toThrow('Invalid cron expression: not-a-cron');
        });
    });
    describe('getStats', () => {
    ...
    describe('priority handling', () => {
        it('should assign correct priority based on agent type', async () => {
            ...
            const customerSupportTask = details.find((d) => d.agentId === 'customer-support-agent');
            const contentCreatorTask = details.find((d) => d.agentId === 'content-creator-agent');
            expect(customerSupportTask?.priority).toBe(AgentScheduler_1.AgentPriority.HIGH);
            expect(contentCreatorTask?.priority).toBe(AgentScheduler_1.AgentPriority.NORMAL);
        });
        it('should use priority from agent configuration when available', async () => {
            ...
            await scheduler.start();
            const details = scheduler.getTaskDetails();
            expect(details[0]?.priority).toBe(AgentScheduler_1.AgentPriority.CRITICAL);
        });
    });
    ...
    describe('error handling and retries', () => {
        ...
        it('should remove agent from schedule after max retries', async () => {
            ...
            mockPrisma.aIAgent.findMany.mockResolvedValue([failingAgent]);
            mockAgentManager.startAgent.mockRejectedValue(new Error('Persistent failure'));
            ...
            const errorStatusUpdate = updateCalls.find((call) => call[0].data && call[0].data.status === 'ERROR');
    ...
    describe('missed jobs handling', () => {
        it('should run missed jobs on startup when enabled', async () => {
            ...
            const schedulerWithMissedJobs = new AgentScheduler_1.AgentScheduler(mockPrisma, mockAgentManager, {
                checkInterval: 1000,
...
//# sourceMappingURL=AgentScheduler.test.js.map