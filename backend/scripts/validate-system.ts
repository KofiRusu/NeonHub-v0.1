...
async function validateSystem() {
  ...
  try {
    ...
    // User authentication
    results.push(await runTest('User Authentication', async () => {
    ...
    // Project data
    results.push(await runTest('Project Data', async () => {
    ...
    // Agent functionality
    results.push(await runTest('Agent Functionality', async () => {
      ...
      if (!agent) {
        return { 
          passed: false, 
          message: 'No content creator agent found. Please run the seed script first.' 
        };
      }
    ...
    // WebSocket server
    results.push(await runTest('WebSocket Server', async () => {
      return await testWebSocketServer();
    }));
    
    // Campaign data
    results.push(await runTest('Campaign Data', async () => {
    ...
    // Generated content
    results.push(await runTest('Generated Content', async () => {
      const contentCount = await prisma.generatedContent.count();
      
      return { 
        passed: contentCount > 0, 
        details: contentCount > 0 ? `${contentCount} content items available` : 'No generated content found',
        message: contentCount === 0 ? 'No content found. This is not critical but demo will be limited.' : undefined
      };
    }));
    
    // Trend signals
    results.push(await runTest('Trend Signals', async () => {
      const signalCount = await prisma.trendSignal.count();
      
      return { 
        passed: signalCount > 0, 
        details: signalCount > 0 ? `${signalCount} trend signals available` : 'No trend signals found',
        message: signalCount === 0 ? 'No trend signals found. This is not critical but demo will be limited.' : undefined
      };
    }));
    
    // Performance check
    results.push(await runTest('Database Performance', async () => {
      ...
      return { 
        ...
        message: !passed ? 'Database query performance is slower than expected.' : undefined
      };
    }));
  ...
  for (const result of results) {
    if (result.passed) {
      console.log(`${chalk.green('✓')} ${chalk.green(result.name)} ${result.details ? chalk.gray(`(${result.details})`) : ''}`);
    } else {
      console.log(`${chalk.red('✗')} ${chalk.red(result.name)} ${result.message ? chalk.gray(`(${result.message})`) : ''}`);
      allPassed = false;
    }
  }
  ...
  console.log(chalk.gray('\n===================================='));
  if (allPassed) {
    console.log(chalk.green('✅ All validation tests passed! System is ready for launch.'));
  } else {
    console.log(chalk.yellow('⚠️ Some validation tests failed. Please review the issues above.'));
  }
}
 ...
 */
async function runTest(name: string, testFn: () => Promise<Omit<TestResult, 'name'>>): Promise<TestResult> {
  ...
  try {
    ...
    return {
      ...
      details: result.details ? `${result.details} (${duration.toFixed(0)}ms)` : `Completed in ${duration.toFixed(0)}ms`
    };
  } catch (error: any) {
    const duration = performance.now() - start;
