...
async function validateDatabaseConnection() {
  ...
  try {
    ...
    throw new Error(
      'Database connection failed. Please check your connection settings in .env',
    );
  }
}
...
async function validateAgentExecution() {
  ...
  try {
    ...
    console.log(
      '⚠️ Agent execution validation failed, but continuing with launch preparation',
    );
  }
}
...
async function generateSampleOutput() {
  ...
  for (const agent of agents) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      endDate.setMinutes(
        endDate.getMinutes() + 3 + Math.floor(Math.random() * 10),
      );
        ...
        data: {
          ...
          logs: {
            entries: [
              ...
              {
                level: 'info',
                message: 'Analyzing campaign context',
                timestamp: new Date(startDate.getTime() + 10000).toISOString(),
              },
              {
                level: 'info',
                message: 'Generating content',
                timestamp: new Date(startDate.getTime() + 45000).toISOString(),
              },
              {
                level: 'info',
                message: 'Content generation complete',
                timestamp: new Date(startDate.getTime() + 90000).toISOString(),
              },
              {
                level: 'info',
                message: 'Storing results in database',
                timestamp: new Date(startDate.getTime() + 110000).toISOString(),
              },
              {
                level: 'info',
...
async function validateWebSockets() {
  ...
  if (
    fs.existsSync(path.join(socketPath, 'index.ts')) &&
