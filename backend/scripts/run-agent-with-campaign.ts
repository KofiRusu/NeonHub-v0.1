...
async function runAgentWithCampaign() {
  try {
    ...
    console.log(campaign ? `Found existing campaign: ${campaign.name}` : 'No existing campaign found');
    ...
    if (result.campaignId) {
      ...
      const resultCampaign = await campaignService.getCampaign(result.campaignId);
