...
import { getSocketIO, emitAgentStart, emitAgentLog, emitAgentDone, emitAgentError } from '../../socket/agentOutput';

/**
 * Abstract base class for all AI agents in the system
 */
export abstract class AIAgent<TConfig extends AgentConfig = AgentConfig, TOutput = any> {
   ...
   */
  public async run(options: AgentRunOptions = {}): Promise<AgentResult<TOutput>> {
    ...
    try {
      ...
      this.log('info', `Agent ${this.id} (${this.type}) completed successfully in ${duration}ms`);
      ...
      this.log('error', `Agent ${this.id} (${this.type}) failed: ${errorMessage}`, { error });
  ...
  protected async updateStatus(status: AgentStatus): Promise<void> {
    ...
    try {
      ...
      this.log('warning', `Failed to update agent status in database: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  protected async saveResult(result: AgentResult<TOutput>): Promise<void> {
    try {
      ...
      */
      ...
      this.log('warning', `Failed to save agent result: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  protected log(level: AgentLogEntry['level'], message: string, context?: Record<string, any>): void {
   ...
   */
  protected async retry(options: AgentRunOptions): Promise<AgentResult<TOutput>> {
    ...
    this.log('info', `Retrying agent execution (${retryCount}/${this.config.maxRetries}) after ${delay}ms delay`);
