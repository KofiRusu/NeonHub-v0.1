...
import { AgentConfig, AgentRunOptions, ContentAgentOutput } from '../base/types';
 ...
 */
export class ContentAgent extends AIAgent<ContentAgentConfig, ContentAgentOutput> {
  /**
   ...
   */
  protected async execute(options: AgentRunOptions): Promise<ContentAgentOutput> {
  ...
  private generateContent(): string {
    ...
    const maxParagraphs = Math.min(paragraphs.length, Math.ceil((length.max || 500) / 100));
