...
import { AgentConfig, AgentRunOptions, OutreachAgentOutput } from '../base/types';
 ...
 */
export class OutreachAgent extends AIAgent<OutreachAgentConfig, OutreachAgentOutput> {
  /**
   ...
   */
  protected async execute(options: AgentRunOptions): Promise<OutreachAgentOutput> {
    ...
    try {
      ...
      const contactMethod = (taskData?.contactMethod as ContactMethod) || ContactMethod.EMAIL;
   ...
   */
  private generateResponse(leadInfo: Record<string, any>, contactMethod: ContactMethod): string {
    ...
    switch (contactMethod) {
      case ContactMethod.EMAIL:
        template = this.generateEmailTemplate(name, company, personalizationLevel);
        break;
      case ContactMethod.LINKEDIN:
        template = this.generateLinkedInTemplate(name, company, personalizationLevel);
        break;
      default:
        template = this.generateGenericTemplate(name, company, personalizationLevel);
    }
   ...
   */
  private generateEmailTemplate(name: string, company: string, personalizationLevel: string): string {
...
The NeonHub Team`;
   ...
   */
  private generateLinkedInTemplate(name: string, company: string, personalizationLevel: string): string {
    return `Hi ${name},
...
The NeonHub Team`;
   ...
   */
  private generateGenericTemplate(name: string, company: string, personalizationLevel: string): string {
    return `Hello ${name},
